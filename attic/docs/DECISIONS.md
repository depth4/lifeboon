...
| Testing against a real town | **Solved.** The site exports the loaded area (About → "Save this place") as one ~0.2 MB gzipped file; `npm run place -- <file>` measures it and the site loads it back with the button beside it. Overpass is still unreachable from the sandbox, but a real town can now be carried there by hand. Within a minute of existing it found a bridge being treated as a junction with the street it flew over — 10.2 m of earth raised under it — which no synthetic scene had produced. |
...
| ~~Junction shape~~ | **Done.** A junction is a place with a boundary now, built constructively from the approach bearings and widths — mouths, rounded corners, one filled surface with a kerb, pavement and earth batter round every corner. It **warps**: streets are levelled to agree at the middle and climb away at their own gradients, so each mouth sits at the height its own street arrives at. The earth is graded to the same warped surface and streets no longer cut through junctions at all. Measured on the offline city: crossings went from 2.9% of samples with earth standing in them to 0.55%, worst 10.2 m to 1.7 m. What is left is the case just outside a junction where two streets are both built and both climbing, and the earth can only follow one. |
